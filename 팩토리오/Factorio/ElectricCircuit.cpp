...
HRESULT ElectricCircuit::Init(int x, int y)
{
	pos.x = x;
	pos.y = y;
	image = ImageManager::GetSingleton()->FindImage("electronic-circuit-icon");
	layer = ELECTRONICCIRCUIT;
	isPicked = false;

	return S_OK;
}

void ElectricCircuit::Release()
{
}

void ElectricCircuit::Update()
{
	tp.x = (int)(pos.x / 32);
	tp.y = (int)(pos.y / 32);
...
void ElectricCircuit::Render(HDC hdc)
{
...
void ElectricCircuit::SelectBoxRender(HDC hdc)
{
}

...
HRESULT ProductCopper::Init(int x, int y)
{
	pos.x = x;
	pos.y = y;
	image = ImageManager::GetSingleton()->FindImage("copper-icon");
	layer = COPPER;
	isPicked = false;
...
void ProductCopper::Release()
{
}

void ProductCopper::Update()
{
	tp.x = (int)(pos.x / 32);
	tp.y = (int)(pos.y / 32);
...
void ProductCopper::Render(HDC hdc)
{
...
void ProductCopper::SelectBoxRender(HDC hdc)
{
}

ProductCopper::ProductCopper()
{
}
